	...
	while(blob.l < blob.length - 1) {
		...
		if(R && R.f) {
			...
			switch(Rn) {
				...
				case 'CalcRefMode': opts.CalcRefMode = val; break;
				case 'Uncalced': break;
...
function sheet_to_json(sheet, cond, opts){
	overlapArray = [];
	...
	for (R = r.s.r + offset; R <= r.e.r; ++R) {
		...
		for (C = r.s.c; C <= r.e.c; ++C) {
			val = sheet[cols[C] + rr];
			if(val === undefined || val.t === undefined) v = '';
			else v = val.v;
			//Validation Check
			var vcr = excelValCheck(v, cond, cols[C]);
			for(var i = 0; i < vcr.length; i++){
				if(typeof vcr[i].success != 'undefined' && !vcr[i].success) {
					vcr[i].col = cols[C] + rr;
					errorList.push(vcr[i]);
				}
			}
			...
			if(v !== undefined) {
				row[cols[C] + rr] = raw ? v : format_cell(val,v);
				isempty = false;
			}
		}
...
function sheet_to_row_object_array(sheet, cond, opts) { return sheet_to_json(sheet, cond, opts != null ? opts : {}); }
...
function excelValCheck(v, cond, colType) {
	var rArray = [];
	var r = {success: true};
	if(typeof cond[colType] == 'undefined') return r;
	var detailCond = cond[colType];
	for(var i = 0; i < detailCond.length; i++) {
		if(typeof detailCond[i] == 'object') {
			switch(detailCond[i].condType) {
				case 'maxLengthCheck' :
					r = excelMaxLengthCheck(v, detailCond[i].value);
					break;
				default :
					r = {success: false, errorType : 'undefined'};
					break;
			}
...
function excelEmptyCheck(v) {
	return v === '' ? {success : false, errorType : 'emptyCheck'} : {success: true};
}

function excelOverlapCheck(v) {
	v = v.toString();
	var r = {success: true};
	for(var i = 0; i < overlapArray.length; i++) {
		if(v != '' && v == overlapArray[i]) {
			r = {success : false, errorType : 'overlapCheck'};
			break;
		}
	}
	if(v != '' && r.success) overlapArray.push(v);
	return r;
}

function excelMaxLengthCheck(v, cv) {
	v = v.toString();
	return v.length > cv ? {success : false, errorType : 'maxLengthCheck'} : {success: true};
}

function excelFloatCheck(v) {
	 return !isNaN(v) && v % 1 !== 0 ? {success: true} : {success : false, errorType : 'floatCheck'} ;
}

function excelPhoneCheck(v) {
	var re = /^[\+\-0-9][\-0-9]+$/gm;
	return re.test(v) ? {success: true} : {success : false, errorType : 'phoneCheck'}; 
}

function excelLatCheck(v) {
	var reg = /^(-)?(?:90(?:(?:\.0{1,6})?)|(?:[0-9]|[1-8][0-9])(?:(?:\.[0-9]{1,15})?))$/;
	var latv = Number(v);
	
	var check = !reg.test( latv );
	
    if( !check ){
        return {success: true};
    }else{
        return {success : false, errorType : 'latCheck'};
    }
}

function excelLngCheck(v) {
   	var reg = /^(-)?(?:180(?:(?:\.0{1,6})?)|(?:[0-9]|[1-9][0-9]|1[0-7][0-9])(?:(?:\.[0-9]{1,15})?))$/;
	var lngv = Number(v);
	
	var check = !reg.test( lngv );
	
    if( !check ){
        return {success : true };
    }else{
        return {success : false, errorType : 'lngCheck'};
    }
}