...
var NodeDataClass = (function () {
    ...
    //상위 task ID
    NodeDataClass.prototype.setParTaskId = function () {
    	var node = getSourceNodeDataByName(this.nodeName);
    	
    	if (isNotEmpty(node)) {
    		this.parTaskId = node.taskId;
    	}
    };
    
    //연결설 유형코드
    NodeDataClass.prototype.setCnctrTypeCd = function () {
    	var type = getInCnctrTypeCdByName(this.nodeName);
    	
    	if (isNotEmpty(type)) {
    		this.cnctrTypeCd = type;
    	}
    };
