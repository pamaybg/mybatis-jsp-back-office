...
function OnMethods(item, handle) {
	...
	//캠페인 기본정보와 연결 확인
	var campaignInfo = item.nodeType == "INFO" ? item : {};
	if (isEmpty(campaignInfo) && isNotEmpty(getParentNodesByNodeType(diagram.selectionList[0], "INFO")[0])) {
		campaignInfo = getParentNodesByNodeType(diagram.selectionList[0], "INFO")[0].addInfo;
	}

	if (isNotEmpty(campaignInfo)) {
		//캠페인 기본정보 저장 확인(캠페인기본정보 이외의 노드일 경우)
		if (isEmpty(campaignInfo.actualTaskId) && item.nodeType != "INFO") {
			alert(MESSAGE.campaign.saveCampaign);
			return false;
		} else {
			//캠페인 상태 설정
			if (isNotEmpty(campaignInfo.data) && isNotEmpty(campaignInfo.data.campaignDefaultInfo)) {
				campaign.statusCode = campaignInfo.data.campaignDefaultInfo.campaignStatusCode;
			}
		}

		//캠페인 ID/상태 설정
        campaign.id = campaignInfo.actualTaskId;

        //현재 노드 캠페인 ID 설정
        diagram._selectedObject.campaignId = campaignInfo.actualTaskId;
	} else {
		//현재 노드 캠페인 ID 설정
        diagram._selectedObject.campaignId = "";
		alert(MESSAGE.campaign.notConnectedCampaign);
    ...
    if (isNotEmpty(method) && isNotEmpty(methodType)) {
        ...
        if (methodType == "POPUP") {
            ...
            var option = {
                    ...
                    open: function (args) {
                        ...
                        if (isNotEmpty(campaign.id)) {
                            ...
                            if (!checkNodeUnauthrized(campaign.id, item.nodeType)) {
                                //roi, monitoring 제외
                                ...
                                if (handle.handleName == "infoPop") {
                                    ...
                                    //저장버튼
                                    //$("#campaignInfoPopSaveBtn").attr('disabled',false);
                                }
            ...
            //이메일 콘텐츠 팝업 일 경우
            else if (handle.handleName == "emailContentsDesignPop") {
                option.width = 1030;
                option.customFunc = 'contentEmailPopupInit';
            }
            //카카오톡 콘텐츠 팝업 일 경우
            else if (handle.handleName == "kakaoContentsDesignPop") {
                option.width = 1500;
                option.customFunc = 'contentKaKaoPopupInit';
            }
            //오퍼 팝업 일 경우
            ...
            //룰셋 원복 팝업
            else if(handle.handleName == "ruleSetRollbasckPop") {
                option.width = 1090;
            }
            else if(handle.handleName == "waitPop") {
                option.width = 480;
            }
            else if(handle.handleName == "respProdPop"){
            	option.width = 1090;
            } 
...
function setDropOnNode(args) {
	...
	if( isNotEmpty(node._type) && node._type == "node" ) {
		...
		if (checkDropTriggerNode(node.addInfo) || checkMultiStageNode(node.addInfo)) {
...
function checkDropTriggerNode(node) {
	...
	if (getCampaignTypeCode() == "TRIGGER" && (node.paletteTypeCode == "EVENT_TRIGGER" || node.paletteTypeCode == "CHANNEL")) {
		flag = true;
	}
...
function checkEventTriggerNode(node) {
	...
	if (getCampaignTypeCode() == "TRIGGER" && node.paletteTypeCode == "EVENT_TRIGGER") {
		flag = true;
	}
...
//멀티스테이지 노드 확인
function checkMultiStageNode(node) {
	var flag = false;
	
	if (getCampaignTypeCode() == "MULTISTAGE" && (node.paletteTypeCode == "MULTI_STAGE" || node.paletteTypeCode == "CHANNEL")) {
		flag = true;
	}
...
function createIntelligentShape(name, fNode) {
    ...
    //노드 생성시 이전 노드와의 Connect
        ...
        if (isNotEmpty(sNode)) {
        	...
        	//캠페인 ID 설정
        	fNode.addInfo.campaignId = sNode.addInfo.campaignId;
        } else {
        	return null;
        }
        ...
        // 소스노드가 멀티스테이지의 룰 노드일 경우 커넥터를 Yes 또는 No 선택
        if ((checkMultiStageNode(sNode.addInfo) || checkEventTriggerNode(sNode.addInfo)) && (sNode.addInfo.nodeType == "MSCOND" || sNode.addInfo.nodeType == "ETCOND")) {
        	lineType = "";
        	fNodeId = fNode.addInfo.nodeId;
    		$("#dialog").ifvsfPopup({
    			enableModal : true,
    			enableResize : false,
    			contentUrl : "/marketing/campaign/popup/campaignMultiStageLineSelectPop",
    			contentType : "ajax",
    			title : "Yes or No",
    			width : "400px",
                open : function() {
                    ...
                    lineSelectPopData.sourceNode = sourceNode;
                    lineSelectPopData.targetNode = fNode.name;
                },
    			close: function() {
    				var node = diagram.getNode(fNode.addInfo.nodeName);
    				diagram.remove(node);
    				dialog._destroy();
    			},
    		});
	        ...
	        //커넥터 생성
	        var item = getPaletteItemByName(lineType);
	        if (item) {
	            var name = $.ifvmGetRowID();//item.name + ej.datavisualization.Diagram.Util.randomId();
...
function setDiagramSize(showpropeditor) {
    ...
    var contentPadding = $(window).width() - $('#content').width() - $("#sideMenu").width();
    //if (!showpropeditor)
    //    pwidth = 0;
    $("#main-right").css("width", width - mwidth - contentPadding);
...
function setNewNodeDataClass(data) {
    ...
    nodeData.paletteTypeCode = data.paletteTypeCode;
...
function getAllNodeDataClass(obj) {
  ...
  $.each(obj.nodes(), function(index, node) {
      if (isNotEmpty(node.addInfo.nodeId)) {
    	  ...
    	  //set Campaign ID
    	  var campaignInfo = getNodesByNodeType(node, "INFO");
    	  if (isNotEmpty(campaignInfo)) {
    	      node.addInfo.campaignId = campaignInfo[0].addInfo.actualTaskId;
    	      node.addInfo.setParTaskId();
    	      node.addInfo.setCnctrTypeCd();
    	  }
...
function setNodeDataByTaskId(data){
    $.each(data, function(index, data) {
    	...
    	if (isNotEmpty(node)){
            ...
            node.addInfo.campaignId = data.campaignId;    //캠페인 ID
            node.addInfo.oldCampaignId = data.campaignId;    //이전 캠페인 ID
            node.addInfo.campaignWfRelId = data.id;    //캠페인 워크플로우 관계 ID
            ...
            if(data.taskTypeCode == "INFO") {
                campaign.id =  data.actualTaskId;
                campaign.statusCode =  data.data.campaignDefaultInfo.campaignStatusCode;
                campaign.templateUseFlag = data.data.campaignDefaultInfo.camTemplateUseYn == "true" ? "Y" : "N";
...
function removeSelectedNodes(args) {
	...
	$.each(diagram.model.selectedItems.children, function(index, node) {
		...
		if (node.addInfo.nodeType == "INFO") {
			removeFlag = false;
			alert(MESSAGE.campaign.canNotRemoveCampaignInfo);
		...
		if (isNotEmpty(campaignInfo) && isNotEmpty(campaignInfo.addInfo.data) && isNotEmpty(campaignInfo.addInfo.data.campaignDefaultInfo)) {
			...
			if (isNotEmpty(statusCode) && statusCode != "W") {
				removeFlag = false;
				alert(MESSAGE.campaign.notWorkingCampaign);
...
function setNewCampaignWorkflowNode() {
    ...
    $.each(node.addInfo.handleList, function(i, e) {
    	if(e.handleName == 'infoPop') {
    		handle.method = PAGE_CONTEXT_PATH + e.method;
    	}
    })
...
function checkAddNode(args) {
	if (isNotEmpty(args.element) && isNotEmpty(args.element.addInfo)) {
		...
		//이벤트 트리거 노드 일 경우 이벤트(트리거) 캠페인만 사용 가능
		if (node.paletteTypeCode == 'EVENT_TRIGGER' && campaignTypeCode != 'TRIGGER') {
			alert(MESSAGE.campaign.selectTriggerType);
		...
		//이벤트 트리거 노드 일 경우 이벤트(트리거) 캠페인만 사용 가능
		if (node.paletteTypeCode == 'MULTI_STAGE' && campaignTypeCode != 'MULTISTAGE') {
			alert("캠페인 유형이 멀티스테이지 캠페인이 아닙니다.");
		...
		if (node.nodeType == "WAIT") {
			if (!(diagram._onMouseenterNode.addInfo.nodeType == "INFO" || diagram._onMouseenterNode.addInfo.nodeType == "WAIT")) {
				alert("Wait 노드는 기본정보 노드 또는 Wait 노드에 연결 가능합니다.");
				...
				if (getChildNodes(diagram._onMouseenterNode, "WAIT").length > 0) {
					alert("Wait 노드가 이미 존재 합니다.");
		...
		//캠페인 기본정보의 프로모션 여부를 체크 해야 프로모션 상세 노드를 추가 할 수 있다.
		if (node.nodeType == 'RULESET' && !getCampaignPromotionFlag()) {
			alert(MESSAGE.campaign.M02723);
		...
		//이벤트 트리거 캠페인 일 경우 프로모션 상세 노드를 추가 할 수 없다.
		if (node.nodeType == 'RULESET' && campaignTypeCode == 'TRIGGER') {
			alert(MESSAGE.campaign.M02724);
		...
		//프로모션 상세 노드가 없을 경우 영수증 채널 노드를 추가 할 수 없다.
		if (node.nodeType == 'RECEIPT' && findNodeByNodeType('RULESET').length == 0) {
			alert(MESSAGE.campaign.M02725);
		...
		//MASS 캠페인 일 경우 타겟팅 노드는 추가 할 수 없다.
		if (node.nodeType == 'TARGET' && campaignTypeCode == 'MASS') {
			alert(MESSAGE.campaign.M02726);
		...
		//B2C이벤트 노드는 캠페인 기본정보의 이벤트 여부가 1 일때만 추가 가능하다
		if (node.nodeType == 'B2C_EVENT' && chkEventYn() == false) {
			alert(MESSAGE.campaign.M02857);
		...
		if (!(checkDropTriggerNode(node) || checkMultiStageNode(node))&& findNodeByNodeType(node.nodeType).length >= 1 ) {
		...
		if (checkDropTriggerNode(node) || checkMultiStageNode(node)) {
...
function getCampaignTypeCode() {
	...
	if ($.fn.ifvmIsNotEmpty(nodes)) {
		...
		if ($.fn.ifvmIsNotEmpty(node.addInfo.data)) {
			if ($.fn.ifvmIsNotEmpty(node.addInfo.data.campaignDefaultInfo)) {
				campaignTypeCode = node.addInfo.data.campaignDefaultInfo.campaignTypeCode;
			}
		}
	}
...
// 상위 노드 데이터 추출
function getSourceNodeData(name) {
	var sourceData;
	if (typeof diagram != "undefined" && isNotEmpty(diagram._selectedObject)) {
		var node = diagram.getNode(diagram._selectedObject.nodeName);
		if (node != null) {
			var cnctr = diagram.getNode(node.inEdges[0]);
			if (isNotEmpty(cnctr)) {
				var sourceNode = diagram.getNode(cnctr.sourceNode);
				if (isNotEmpty(sourceNode)) {
					sourceData = sourceNode.addInfo;
				}
			}
		}
	}
...
//상위 노드 데이터 추출
function getSourceNodeDataByName(name) {
	var sourceData;
	var node = diagram.getNode(name);
	if (node != null) {
		var cnctr = diagram.getNode(node.inEdges[0]);
...
//상위 노드 데이터 추출
function getInCnctrTypeCdByName(name) {
	var type;
	var node = diagram.getNode(name);
	if (node != null) {
		var cnctr = diagram.getNode(node.inEdges[0]);
		if (isNotEmpty(cnctr)) {
			type = cnctr.addInfo.title;
			if (isNotEmpty(type)) {
