...
var Grid = fc.Grid = RowRenderer.extend({
	...
	headCellHtml: function(cell) {
		...
		var date = cell.start;
		var dday = '';
		//var dday = htmlEscape(date.format(this.colHeadFormat));
		
		switch( htmlEscape(date.format(this.colHeadFormat)) ){
...
Grid.mixin({
	...
	getSegClasses: function(seg, isDraggable, isResizable) {
		...
		classes.push(event.colorClass);
		classes.push(event.campaignId);
		if (isDraggable) {
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		Number.prototype.add0 = function(){return this<10?'0'+this:this;};
		 
		var startDateVal = new Date(event.start);

		var startDate = startDateVal.getDate();
		var startMonth = (startDateVal.getMonth() + 1).add0();
		var startYear = startDateVal.getFullYear();
		var startDay = startDateVal.getDay();
		
		var endDateVal = new Date(event.end);
		 
		var endDate = endDateVal.getDate();
		var endMonth = (endDateVal.getMonth() +1).add0();
		var endYear = endDateVal.getFullYear();
		 
		var st = startYear+""+startMonth+""+startDate;
		var ed = endYear+""+endMonth+""+endDate;
		//alert("startDate : " + startYear+startMonth+startDate+","+"endDate" +":" +endYear+endMonth+endDate);
		...
		if (seg.isStart) {
			...
			if (timeText) {
				//timeHtml = '<span class="fc-time">' + htmlEscape(timeText) + '</span>';		//time이벤트 표시 ex) 12a
			}
		}
		...
		titleHtml =
			...
			'&nbsp;' +'&nbsp;' +
			 ...
			 '&nbsp;' + '&nbsp;' +'&nbsp;' +'&nbsp;' +
			 (htmlEscape(event.campaignStatusCodeName || '') || '&nbsp;') +// we always want one line of height
			'</span>';
...
Calendar.defaults = {
	...
	monthYearFormat: 'YYYY.MM', // required for en. other languages rely on datepicker computable option
...
function convertK(title) {
	var cTitle = title.split(' ');
	var cDate = '';
	if(cTitle[2] == 'Su')
		cDate = MESSAGE.calendar.sunday;
	else if(cTitle[2] == 'Mo')
		cDate = MESSAGE.calendar.monday;
	else if(cTitle[2] == 'Tu')
		cDate = MESSAGE.calendar.tuesday;
	else if(cTitle[2] == 'We')
		cDate = MESSAGE.calendar.wednesday;
	else if(cTitle[2] == 'Th')
		cDate = MESSAGE.calendar.thursday;
	else if(cTitle[2] == 'Fr')
		cDate = MESSAGE.calendar.friday;
	else if(cTitle[2] == 'Sa')
		cDate = MESSAGE.calendar.saturday;
	
	return cTitle[0] + ', ' + cTitle[1] + ' ' + cDate;
}