                        ...
                        if (isNotEmpty(campaign.id)) {
                            ...
                            if (!checkNodeUnauthrized(campaign.id, item.nodeType)) {
                                ...
                                if (handle.handleName == "infoPop") {
                                    ...
                                    //hierarchy select box 이벤트 꼬이는 관계로 기본정보팝업 데이터 한번 더 세팅한다.
...
//            console.log(handle.handleName);
           ...
           //카카오톡 콘텐츠 팝업 일 경우
            else if (handle.handleName == "kakaoNotiContentsDesignPop") {
                option.width = 2500;
            ...
            else if (handle.handleName == "actionPop") {
                option.width = 680;
            }
          //캠페인 기본정보
            else if (handle.handleName == "rulePop") {
                option.width = 1450;
                //option.hide = hide;
            }
          //캠페인 기본정보
            ...
            else if (handle.handleName == "kpiPop") {
                option.width = 680;
            }
...
function checkDropTriggerNode(node) {
	...
	if (getCampaignTypeCode() == "TRIGGER" && (node.nodeType == "ETCOND" || node.nodeType == "ETACT" || checkChnlNode(node.nodeType))) {
		flag = true;
	}
    ...
    $.each(data, function(index, data) {
    	...
    	if (isNotEmpty(node)){
            ...
            if(data.taskTypeCode == "INFO") {
                ...
                campaign.templateUseFlag = data.data.campaignDefaultInfo.camTemplateUseYn == "true" ? "1" : "0";
...
function removeSelectedNodes(args) {
	...
	$.each(diagram.model.selectedItems.children, function(index, node) {
		...
		if (node.addInfo.nodeType == "INFO") {
			removeFlag = false;
			alert(MESSAGE.campaign.canNotPromotionInfo);
...
function removeAllNodes() {
	...
	if (removeFlag) {
		...
		var tNodes = [];
		$.each(diagram.model.nodes, function(index, node) {
				var nodeName = diagram.getNode(node.addInfo.nodeName);

				if (node.addInfo.nodeType != "INFO" && node.addInfo.nodeType != "ETCOND" && node.addInfo.nodeType != "ETACT" ) {
...
function checkAddNode(args) {
	if (isNotEmpty(args.element) && isNotEmpty(args.element.addInfo)) {
		...
		var notiYnCode = getCampaignNotiYn();
		var promTypeLv2Cd = getCampaignGenYn();
		//이벤트 트리거 노드 일 경우 이벤트(트리거) 캠페인만 사용 가능
		/*if (checkEventTriggerNode(node) && campaignTypeCode != 'TRIGGER') {
		...
		//알림 사용 유무에 따라서 채널 등록 제약
		if(checkChnlNode(node.nodeType) && promTypeLv2Cd == 'PUR_GEN'){
			   alert(MESSAGE.campaign.M03030);
		...
		if(checkChnlNode(node.nodeType) && notiYnCode == 'N'){
		   alert(MESSAGE.campaign.M03027);
...
function getCampaignNotiYn() {
	...
	if ($.fn.ifvmIsNotEmpty(nodes)) {
		...
		if ($.fn.ifvmIsNotEmpty(node.addInfo.data)) {
			if ($.fn.ifvmIsNotEmpty(node.addInfo.data.campaignDefaultInfo)) {
				rtnValue = node.addInfo.data.campaignDefaultInfo.notiYn;
			}
		}
	}
...
function getCampaignGenYn() {
	...
	if ($.fn.ifvmIsNotEmpty(nodes)) {
		...
		if ($.fn.ifvmIsNotEmpty(node.addInfo.data)) {
			if ($.fn.ifvmIsNotEmpty(node.addInfo.data.campaignDefaultInfo)) {
				rtnValue = node.addInfo.data.campaignDefaultInfo.promTypeLv2Cd;
			}
		}
	}
