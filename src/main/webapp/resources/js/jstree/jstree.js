 ...
 */
	...
	$.jstree.core.prototype = {
		...
		_parse_model_from_html : function (d, p, ps) {
			...
			data.state.red = d.children('a').hasClass('jstree-red');
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			node.id = obj.id;
			node.className = c;
			c = ( obj.state.selected ? ' jstree-clicked' : '') + ( obj.state.disabled ? ' jstree-disabled' : '') + ( obj.state.red ? ' jstree-red' : '');
			for(j in obj.a_attr) {
					...
					.on('model.jstree', $.proxy(function (e, data) {
							...
							if(s.indexOf('down') !== -1) {
								...
								if(p.state[ t ? 'selected' : 'checked' ]) {
									...
									//20150618 백상주 체크박스 체크 / 텍스트 색상 변화 추가
									for(i = 0, j = dpc.length; i < j; i++) {
										var chdobj = m[dpc[i]];
										if(chdobj.original.hasOwnProperty('progId')) {
											var opid = chdobj.original.progId;
											if(opid === null) {
												chdobj.state['red'] = true;
												this._data['core'].red.push(chdobj.id);
												tmp = this.get_node(chdobj, true);
												
												if(tmp && tmp.length) {
													tmp.attr('aria-selected', true).find('.jstree-anchor').addClass('jstree-red');
												}
											}
										}
										if(chdobj.original.hasOwnProperty('menuId')){
											var omid = chdobj.original.menuId;
											if(omid !== null) {
												chdobj.state[ t ? 'selected' : 'checked' ] = true;
												this._data[ t ? 'core' : 'checkbox' ].selected.push(chdobj.id);
												tmp = this.get_node(chdobj, true);
												if(tmp && tmp.length) {
							...
							if(s.indexOf('up') !== -1) {
								...
								for(k = 0, l = chd.length; k < l; k++) {
									...
									while(p && p.id !== '#') {
										...
										//20150618 백상주 체크박스 체크 / 텍스트 색상 변화 추가
										if(p.original.hasOwnProperty('progId')) {
											var opid = m[chd[k]].original.progId;
											if(opid === null) {
												p.state['red'] = true;
												this._data['core'].red.push(p.id);
												this.get_node(chdobj, true).find('.jstree-anchor').addClass('jstree-red');
												...
												var isChdProgNull = true;
												for(var pi = 0; pi < p.children_d.length; pi++) {
													if(p.children_d[pi].original.progId !== null)
														isChdProgNull = false;
												}
												console.log(isChdProgNull);
												//자식노드 progId가 모두 NULL일 경우 부모에게 텍스트 색상 변화
												if(isChdProgNull){
													p.state['red'] = true;
													this._data['core'].red.push(p.id);
													this.get_node(chdobj, true).find('.jstree-anchor').addClass('jstree-red');
										...
										if(p.original.hasOwnProperty('menuId')){
											var omid = m[chd[k]].original.menuId;
											if(omid !== null) {
