...
//         	defaultSearchClose();
        ...
        colModel:[
             ...
             { name:'mbrLockYn',   index:'a.mbr_lock_yn', 	resizable : true, align:'center',},
...
function mbrSearchHelpSearchInit() {
    $.fn.ifvmSetCommonCondList("mbrSearchHelpSearchList","PGM_L_111");
    searchInitialize();
}
...
// 		mbrSearchHelpPop.close(mbrsel_obj);
		$('#rcptAftrRgstDetailForm #ridMbr').val(obj.rid);
		$('#rcptAftrRgstDetailForm #mbrNo').val(obj.mbrNo);
		$('#rcptAftrRgstDetailForm #custNm').val(obj.custNm);
		$('#rcptAftrRgstDetailForm #mbrStatCdNm').val(obj.mbrStatNm);
		if(obj.mbrLockYn === 'Y') {
			$('#rcptAftrRgstDetailForm #mbrLockYn').prop('checked', true);
		} else {
			$('#rcptAftrRgstDetailForm #mbrLockYn').prop('checked', false);
		}
		
		mbrSearchHelpPopupClose();
	}
	else {
...
<%-- 		<ifvm:inputNew type="button"   text="M00441"  id="cancelMbrSearchHelpBtn" btnFunc="cancelMbrSearchHelpFn"/> --%>
		<ifvm:inputNew type="button"   text="M00441"  id="cancelMbrSearchHelpBtn" btnFunc="mbrSearchHelpPopupClose"/>
	</div>
</div>