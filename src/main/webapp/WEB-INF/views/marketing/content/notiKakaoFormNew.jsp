        ...
        if (conid != null) {
            ...
            $.ifvSyncPostJSON('<ifvm:action name="getDescContsKakao"/>', {
                ...
                function (result) {
                    ...
                    $('#templateCd').val(result.attrib03)                        //템플릿코드
                    ...
                    templateCdData = result.attrib03;
        ...
        if (selectForm.confirm() && phoneForm.confirm()) {
            ...
            if (conid == null) {
                ...
                if (typeof closeContentsDesignPop == 'function') {
                    ...
                    qtjs.href('/marketing/content/notiKakaoFormNew' + '?id=' + conid);
                }
                // 	update
            } else {
    ...
    function testBtnPopup() {
        ...
        if (conid != null) {
            $("#testbtnPop").ifvsfPopup({
                enableModal: true,
                enableResize: false,
                contentUrl: '<ifvm:url name="notiKaKaoFormNewTestBtnPop"/>',
                contentType: "ajax",
                title: '테스트 발송',
                width: 1024,
    ...
    function ofrListPop() {
        ...
        if (openType == "WF" && typeof campaign != "undefined") {
            $.ifvSyncPostJSON('<ifvm:action name="getCampaignOfferCount"/>', {
                ...
                function (result) {
                    if (result > 0) {
                        $.ifvSyncPostJSON('<ifvm:action name="getCampaignLoadOfferPopCount"/>', {campaignId: campaign.id, chnnelTypeCode: 'KAKAONOTI',contentsId: conid},
                            function (result) {
                                if (result > 0) {
                                    openflag = false;
                                }
    ...
    function ofrPointListPop() {
        ...
        if (openType == "WF" && typeof campaign != "undefined") {
            $.ifvSyncPostJSON('<ifvm:action name="getCampaignOfferCount"/>', {
                ...
                function (result) {
                    if (result > 0) {
                        $.ifvSyncPostJSON('<ifvm:action name="getCampaignLoadOfferCount"/>', {campaignId: campaign.id, chnnelTypeCode: 'KAKAONOTI',contentsId: conid},
                            function (result) {
                                if (result > 0) {
                                    openflag = false;
                                }
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="col-xs-6">
                            <%-- 템플릿 코드 --%>
                            <label class="col-xs-3 control-label"><span class="asterisk">*</span>
                                <spring:message code="I02128"/></label>
                            <div class="col-xs-9 control_content">
                                <ifvm:input type="text" required="true" id="templateCd" dto=""/>
                            </div>
                        </div>
                        <div class="col-xs-6">
                            <%-- SENDER KEY --%>
