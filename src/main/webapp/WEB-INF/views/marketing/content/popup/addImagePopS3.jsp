...
<script>
    ...
    var sizeStore = [];
    var imgFolderPath = "content_images/" + conChanneltype;
    ...
    function obj(){
        ...
        var conimagetype = 'USERCONTS';
        if (typeof contsimagetype != 'undefined') {
            conimagetype = contsimagetype;
        }

        var ajaxSettings = {
            upload: {
                url: "/common/uploadImageFTP.do?imageType=" + conimagetype
            },
            download: {
        ...
        var localServ = "/common/getFileFTP.do";
        ...
        $("#fileExplorer").ejFileExplorer({
            ...
            layout: "list",
            //path : '<ifvm:url name="imageView"/>',
			path : '<%= BeansUtil.getApplicationProperty("icignal.context.imageServerUrl")%>' + imgFolderPath,
            select: "onSelect",
            remove : "onRemove",
            //open : "oOpen",
            ...
            beforeAjaxRequest: function (args) {
                args.data.imageType = conimagetype;
                args.ajaxSettings.successAfter = function (args) {
	                var fileObj = $("#fileExplorer").data("ejFileExplorer");
	                fileObj._sorting("dateModified", false);
	                fileObj._sortingActioncomplete();
                }
            }            
    ...
    function onRemove(args){
        $.ifvSyncPostJSON('<ifvm:action name="removeImageFTP"/>', {
    ...
    function onSelect(args) {
        if(args.selectedItems[0] != null)
        {
            ...
            $('#imgPath').val('<%= BeansUtil.getApplicationProperty("icignal.context.imageServerUrl")%>' + imgFolderPath + '/' + imgName);
            $('#filedate').val(date[0]);
    ...
    function selectImage() {
        ...
        mergeCheck = false;
        var imgFullPath = '<%= BeansUtil.getApplicationProperty("icignal.context.imageServerUrl")%>' + imgFolderPath + '/' + imgName;
        ...
        if(sizeCheck){
            ...
            if(conChanneltype == "mms" || conChanneltype == "null" || conChanneltype == "push"){
    ...
    <div class="well form-horizontal well_sum fileexplorer_bg">
        <%-- <div class="row qt_border">
            ...
            <div class="col-xs-1 text-right">
                <button class="inner_btn" objCode="addImagePopS3Inner_OBJ"><spring:message code="M00811"/></button>
            </div>
        </div> --%>
        <div class="row">
            <label class="col-xs-2 control-label"><spring:message code="M02908"/></label>
            <div class="col-xs-9">
                <ifvm:input type="text" id="imgPath"/>
            </div>
        </div>
        <div class="row">
...
<div class="pop_btn_area">
    <button class="btn btn-sm btn_gray" id="addImage" objCode="addImagePopS3Save_OBJ">
        <i class="glyphicon glyphicon-check"></i>
        <spring:message code="M00282"/>
    </button>
    <button class="btn btn-sm btn_lightGray2" id="fileExplorerClose" objCode="addImagePopS3Close_OBJ">
