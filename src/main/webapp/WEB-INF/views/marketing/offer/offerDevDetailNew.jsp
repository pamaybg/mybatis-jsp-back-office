...
$(function() {
   ...
   // $.fn.ifvmSetSelectOptionCommCode("offerAplyDdTypeCd","OFFER_APLY_DD_TYPE",null,null,true);
...
function offerCheck(){
	...
	}else if(offerTypeCd == 'PNT_COUPON'){
...
function offerTypeCheck(){
	var offerTypeVal = $("#offerTypeCd").val();
	
	if(offerTypeVal == 'POINT'){
		$('#offerAplyDay').val('');
		$('#aplyStartDd').val('');
		$('#aplyEndDd').val('');
		$('#selfTotAmt').val('');
		...
		$('#offerNotiTwo').val('');
		$('#offerNotiThree').val('');
	}else if(offerTypeVal == 'PNT_COUPON'){
		setChannelCalendar();
...
<div>
	<div class="page_btn_area" id="headForm">
	    ...
	    <div class="col-xs-5 searchbtn_r">
	        <%-- 저장버튼 --%>
	        <ifvm:inputNew type="button" btnFunc="saveOffer" text="M00573" id="btnoffersave" objCode="btnoffersave_OBJ"/>
	    </div>
	</div>
