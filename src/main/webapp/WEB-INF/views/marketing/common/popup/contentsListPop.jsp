...
function contentsListPopGrid(){
    ...
    colModelArry.push({name:'contentsChannel',  index:'c1.mark_name',   resizable : false});
    colModelArry.push({name:'contentsName',     index:'mc.CONTS_NM',    resizable : false});
    
	//SMS의 경우 제목 없음. 
	if (chnlType != 'SMS' 
	        && chnlType != 'RECEIPT') {
    	colNameArry.push('<spring:message code="M00377"/>');
        colModelArry.push({name:'title',            index:'mc.CONTS_TITLE', resizable : false});
	}
	...
	colModelArry.push({name:'modifyBy',         index:'c2.name',        resizable : false});
	colModelArry.push({name:'modifyDate',       index:'mc.modify_date', resizable : false});
...
<div id="contentsListArea">
	<div class="pop_inner_wrap">
		<div id="contentsListPopGrid" class="con_size"></div>
	</div>
