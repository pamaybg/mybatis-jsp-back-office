...
function getTableData(data, pYmonthcd, pMemtiercd, pRtnType){
	...
	for(var i=0; i<data.length; i++){
		if(data[i].ymonthcd == pYmonthcd && data[i].memtypecd == pMemtiercd){
			rtnVal = data[i];
			break;
		} 
	}
...
function makeTable(dataArr, ymonth, custTermNm, custTermCd){
	var tableHead = "";
	var tableBody = "";
	var custTypeVal = $("#searchCustType").val();
	var pTermNmArr = termSetting("searchCustType", "nm");
	var pTermCdArr = termSetting("searchCustType", "cd");
	...
	tableHead += " <tr> ";
	tableHead += " 	<th>" + "<spring:message code='M02753'/>" + "</th> ";
	...
	if(custTypeVal == "" || custTypeVal == null){
		//전체
...
function searchFunc(){
	getAnalysisListByCustType();
}

$(document).ready(function(){
	dateSetting("searchYear","searchYear_temp","searchMonth","searchMonth_temp");
	
// 	$.ifvGetCommCodeList('select', 'searchCustType', '<ifvm:action name="getCommCodeList2"/>', 'ACC_MEM_TYPE', 'com', false, false, true);
	$.ifvGetCommCodeList('select', 'searchCustType', '<ifvm:action name="getCommCodeList2"/>', 'BI_MEM_TYPE', 'com', false, false, true);
});
...
<div class="well form-horizontal underline" id="comSearchArea">
	<div class="row qt_border">
		...
		<ifvm:inputNew type="select" id="searchCustType" names="searchCustType" label="M02753" labelClass="1" conClass="2" />
	</div>
</div>
