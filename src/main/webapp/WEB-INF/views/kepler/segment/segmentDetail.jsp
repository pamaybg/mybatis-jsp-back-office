...
var segmentDifferentSavePopupContainer;
var Differentdata = {};

//세그먼트 다른 이름으로 저장 팝업 닫기
function segmentDifferentSavePopClose() {
	segmentDifferentSavePopupContainer._destroy();
}

//세그먼트 다른 이름으로 저장 팝업 열기
function saveAsDifferentSegmentPop() {
	$("#segmentDifferentSavePopupContainer").ifvsfPopup({
		enableModal : true
		,enableResize: false
		,allowKeyboardNavigation : false
		,contentUrl: '<ifvm:url name="segmentDifferentSavePop"/>'
		,contentType: "ajax"
		,title: '<spring:message code="M01428" />'
		,width: 600
		,close : 'segmentDifferentSavePopClose'
	});
}

//세그먼트 다른 이름으로 저장
function saveAsDifferentSegment() {
    var check = false;
	var filterList = [];
	$.each(diagram.nodes(), function(index, node) {
		var fData = {};
		fData.segFilterId = node.name;
		fData.combFilterAId = node.addInfo.data.combFilterAId;
		fData.combFilterBId = node.addInfo.data.combFilterBId;
		filterList.push(fData);
	});

	Differentdata.filterList = filterList;

	$.ifvProgressControl(true);
	$.ifvPostJSON('<ifvm:action name="saveAsDifferentSegment"/>', Differentdata
			, function(result) {
				$.ifvProgressControl(false);
...
$(document).ready(function() {
	...
	//다른이름으로 저장 버튼
	$('#saveAsDifferentSegmentBtn').on('click', function() {
		saveAsDifferentSegmentPop();
	});
	//저장 버튼
...
<div class="page-title">
	<h1>
		<spring:message code="M01469" /> &gt; <spring:message code="M00136" />
	</h1>
</div>
<div>
<div class="page_btn_area">
	...
	<div class="col-xs-5 searchbtn_r" >
		<button class="btn btn-sm" id="saveAsDifferentSegmentBtn" objCode="saveAsDifferentSegmentBtn_OBJ">
...
<div id="segmentDifferentSavePopupContainer" class="popup_container"></div>
