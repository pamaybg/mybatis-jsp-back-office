...
<script type="text/javascript">
    ...
    function prodListSearchInit(gridId) {
        $.fn.ifvmSetCommonCondList("prodSearchList","PRGO_PRODUCT",'prodListNotGrid');
    }

    //공통 조회 호출
    function prodListSearch() {
        $.fn.ifvmSubmitSearchCondition("prodSearchList", function () {
            prodListNotGrid.searchGrid({
                item : requestitem
            });
        });
    }
...
//                 }
        ...
        prodListNotGrid = $("#prodListNotGrid").ifvsfGrid({ ejGridOption : ejGridOption });
    };
    
    
    function saveOfferLoyProd() {
        if(prodListNotGrid.opt.gridControl.getSelectedRecords().length==0){
...
//          getProdSpec();
        ...
        $("#prodListSearchInitBtn").on('click', function(){
            prodListSearchInit("prodListNotGrid");
        });
