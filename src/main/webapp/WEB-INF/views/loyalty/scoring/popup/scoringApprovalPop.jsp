...
function approvalPopupInitData() {
    ...
    var scoringInfo = getNodesByNodeType(diagram.selectionList[0], "INFO");
    if (isNotEmpty(scoringInfo)) {
        ...
        function(result) {
            if ($.fn.ifvmIsNotEmpty(result)){
            	scoringApprovalData.scoringInfo = result;
            }
        });
...
function loadApproval() {
    ...
    //call 전자결재 조회
    var id = "null";
    $.ifvSyncPostJSON('<ifvm:action name="getScoringApproval"/>', {
        scoringId: scoring.id
    },
    function(result) {
        if ($.fn.ifvmIsNotEmpty(result)) {
            id = result.approvalId;
            scoringApprovalData.scoringApproval = result;
        }
    });
    
    var obj = $.ifvCachedHtml('<ifvm:url name="loadScoringApprovalDetail"/>?&id='+id);
    $('#approvalPopCon').html(obj.responseText);
    
    //승인대상 캠페인 지정
    $("#scrId").text(scoringApprovalData.scoringInfo.id);
    $("#scrId").val(scoringApprovalData.scoringInfo.scrNm);
    $("#scrContent").val(scoringApprovalData.scoringInfo.scrDesc);
    $("#scrContent").attr("disabled","true");
...
function approvalPopupSaveBtn() {
    ...
    if ($.fn.ifvmIsNotEmpty(approvalId) && approvalId != "null") {
    	scoringApprovalData.scoringApproval.approvalId = approvalId;
    	
    	diagram._selectedObject.taskType = diagram._selectedObject.nodeType;
    	diagram._selectedObject.data.scoringApproval = scoringApprovalData.scoringApproval;
    	diagram._selectedObject.taskDataStatus = getTaskStatusCode(scoringApprovalData.scoringApproval);
    }
...
<div id="dialogPopupWrap">
	...
	<div class="pop_btn_area">
	    <button class="btn btn-sm btn_gray" id="approvalPopupSaveBtn">
	    ...
	    <button class="btn btn-sm btn_lightGray2 cancel" id="approvalPopupCancelBtn">
