...
function getSettleMbrExctPointExList(){
    var ejGridOption = {
      	serializeGridData : function( data ){
       	...
       	dataUrl : '<ifvm:action name="getSettleMbrExctPointDetailAList"/>',
        columns:[ 
        	  {
	      		  ...
	      		  field : 'acrlAmt', headerText : '<spring:message code="L02435"/>', headerTextAlign : 'center', textAlign : 'right', format : '{0:n0}',
	      		  customAttributes : {
	      			  index : 'acrlAmt' , sortable: false}/* 적립 포인트 */
	      	  },{
	      		  field : 'rdmAmt', headerText : '<spring:message code="L02438"/>', headerTextAlign : 'center', textAlign : 'right', format : '{0:n0}',
	      		  customAttributes : {
	      			  index : 'rdmAmt' , sortable: false}/* 사용 포인트 */
	      	  },{
	      		  field : 'expireAmt', headerText : '<spring:message code="L02425"/>', headerTextAlign : 'center', textAlign : 'right', format : '{0:n0}',
	      		  customAttributes : {
	      			  index : 'expireAmt' , sortable: false}/* 소멸 포인트 */
	      	  }
        ],
        requestGridData : {
	      	  nd   : new Date().getTime(),
...
function getSettleMbrExctPointAList(){
    var ejGridOption = {
        ...
        columns:[ 
        	  {
	      		  field : 'yyyyMm', headerText : '<spring:message code="L02436"/>', headerTextAlign : 'center', textAlign : 'center',
	      		  customAttributes : {
	      			  index : 'yyyyMm' }/* 일자 */
	      	  },{
	      		  ...
	      		  field : 'acrlChnlNo', headerText : '<spring:message code="D10173"/>', headerTextAlign : 'center', textAlign : 'center',
	      		  customAttributes : {
	      			  index : 'acrlChnlNo'}/* 매장 코드 */
	      	  },{
	      		  field : 'acrlchnlNm', headerText : '<spring:message code="L02275"/>', headerTextAlign : 'center',
	      		  customAttributes : {
	      			  index : 'acrlchnlNm'}/* 매장명 */
	      	  },{
	      		  field : 'acrlRcptNo', headerText : '<spring:message code="L00381"/>', headerTextAlign : 'center', textAlign : 'center',
	      		  customAttributes : {
	      			  index : 'acrlRcptNo'}/* 영수증 번호 */
	      	  },{
	      		  field : 'camType', headerText : '캠페인 코드', headerTextAlign : 'center', textAlign : 'center', width : '180px',
	      		  customAttributes : {
	      			  index : 'camType'}/* 캠페인 타입 */
	      	  },{
	      		  field : 'acrlAmt', headerText : '<spring:message code="L02435"/>', headerTextAlign : 'center', textAlign : 'right', format : '{0:n0}',
	      		  customAttributes : {
	      		  ...
	      		  field : 'expireAmt', headerText : '<spring:message code="L02439"/>', headerTextAlign : 'center', textAlign : 'right', format : '{0:n0}',
	      		  customAttributes : { 
	      			  index : 'expireAmt'}/* 소멸 예정 금액 */
	      	  }
        ],
        requestGridData : {
	      	  nd   : new Date().getTime(),
	...
	$(".SearchCondition .searchTextVal option:selected").filter(function(){
		if($(this).attr('jointgtclm') == table){
			if($(this).parents('.SearchCondition').css("display") !="none"){
				if( $.fn.ifvmIsNotEmpty(  $(this).parents('.SearchCondition').find('.changeTypeAreaCode  input').val()) ||
						$.fn.ifvmIsNotEmpty(  $(this).parents('.SearchCondition').find('.changeTypeAreaCode  select').val())) {
					validationVal = true;
				}
			}
		}
...
function settleMbrExctPointListSearchInit() {
    $.fn.ifvmSetCommonCondList("settleMbrExctPointSearchList","SETTLE_MBR_EXT_POINT_02");
...
function settleMbrExctPointAListSearch() {
	if(validationCheck("YYYY") && validationCheck("MM") && validationCheck("MBR_NO")){
		$.fn.ifvmSubmitSearchCondition("settleMbrExctPointSearchList",function() {
			settleMbrExctPointExctAListGrid.searchGrid({item:requestitem,layout:"LIST"});
			settleMbrExctPointExctGrid.searchGrid({item:requestitem,layout:"TOTAL"}); 
		});		
	}else{
		alert("모든 조회조건을 입력해주세요.");
	}
...
function settleMbrExctPointListExcelExport() {
	 $.ifvExcelDnMultiSheet('/loyalty/settle/getSettleMbrExctPointDetailExeDown.do',['settleMbrExctPointExctGrid','settleMbrExctPointExctAListGrid']);
}
