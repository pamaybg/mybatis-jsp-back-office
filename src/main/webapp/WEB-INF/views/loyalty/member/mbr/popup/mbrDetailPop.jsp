		...
		<div id="baseForm">
			<div class="row qt_border" id="rowArea1">
				...
				<div id="cust_nm_addArea">
					<ifvm:inputNew type="text" id="cust_nm_add"  names="cust_nm_add" dto="cust_nm_add" label="고객명(별칭)" labelClass="1" conClass="3" />
				</div>
				<div id="gen_cdArea">
			...
			<div class="row qt_border" id="rowArea2">
                ...
                <div id="telArea">
                <label class="control-label col-xs-1">자택전화</label>
				...
				<div id="emailArea">
					<ifvm:inputNew type="text" id="email" names="email" dto="email" label="이메일" labelClass="1" conClass="7" />
			...
			<div class="row qt_border" id="rowArea3">
				<div id="zipArea">
					<ifvm:inputNew type="search" id="zip" dto="zip"  btnFunc="addrSearchBtn"  conClass="3" label="주소" labelClass="1"/>
				</div>
				<div id="adresArea">
			...
			<div class="row qt_border" id="rowArea4">
				<div id="bunjiArea">
					<ifvm:inputNew type="text" id="bunji" names="bunji" dto="bunji"  label="" labelClass="1" conClass="11" maxLength="50"/>
			...
			<div class="row qt_border" id="rowArea5">
				<div id="wrc_nmArea">
					<ifvm:inputNew type="text" id="wrc_nm" names="wrc_nm" dto="wrc_nm" label="직장명" labelClass="1" conClass="2"  />
					<ifvm:inputNew type="text" id="dept_nm" names="dept_nm" dto="dept_nm" label="부서명" labelClass="1" conClass="2"  />
				</div>
				<div id="empNoArea">
...
<!-- 				</div> -->
			...
			<div class="row qt_border" id="rowArea6">
				<div id="zip_addArea">
					<ifvm:inputNew type="search" id="zip_add" dto="zip_add"  btnFunc="addrSearchBtn2"  conClass="3" label="직장주소" labelClass="1"/>
				</div>
			...
			<div class="row qt_border" id="rowArea7">
				<div id="bunji_addArea">
					<ifvm:inputNew type="text" id="bunji_add" names="bunji_add" dto="bunji_add"  label="" labelClass="1" conClass="11" maxLength="25"/>
			...
			<div class="row qt_border" id="rowArea8">
				<div id="mrrg_ynArea">
					<ifvm:inputNew type="select" id="mrrg_yn" names="mrrg_yn" dto="mrrg_yn" label="결혼여부" labelClass="1" conClass="1" />
				</div>
				<div id="child_ynArea">
                    <ifvm:inputNew type="select" id="child_yn" names="child_yn" dto="child_yn" label="자녀유무" labelClass="1" conClass="1" />
                </div>
				<div id="annvrsry_type_cdArea">
                    <ifvm:inputNew type="select" id="annvrsry_type_cd" names="annvrsry_type_cd" dto="annvrsry_type_cd" label="기념일유형" labelClass="1" conClass="3"/> <!--  disabled="true"  -->
                </div>
                <div id="annvrsryArea">
...
<!-- 				</div> -->
				<div id="drmncy_chng_unitArea">
                    <ifvm:inputNew type="select" id="drmncy_chng_unit" names="drmncy_chng_unit" dto="drmncy_chng_unit" label="휴면전환" labelClass="1" conClass="5" /> <!-- 휴면예정일 -->
...
<%-- 	                                           <ifvm:inputNew type="search" id="prStoreNm" names="prStoreNm" dto="prStoreNm" label="주거래매장" title="주거래매장" btnFunc="prStoreNmFun" labelClass="1" conClass="3" /> --%>
                                               <ifvm:inputNew type="select" id="snsTypeCd" names="snsTypeCd" dto="snsTypeCd"   label="SNS" title="SNS 유형" labelClass="1" conClass="4"/>
                                               <ifvm:inputNew type="text" id="snsAddr" dto="snsAddr"  title="SNS URL" conClass="7"/>
         </div>
		<div class="" id="hideArea"> <!-- 필요하지만 화면상 보여줄 필요가 없는 것들 -->
				...
				<ifvm:inputNew type="select" id="custstatcd" names="custstatcd" dto="custstatcd" label="M02630" labelClass="1" conClass="1"  disabled="true"/>
...
var editMbrDetialPop = function(mbrRid){
	...
	this.getMbrDetail = function(){
		$.ifvSyncPostJSON('<ifvm:action name="getMbrDetailPop"/>',{rid : _mbrRid}, function(result){
	        ...
	        $.fn.ifvmSetSelectOptionCommCode("custstatcd", "LOY_CUST_STAT_CD", result.custStatCd, null, true);
	        ...
	        //[기존데이터로드부분]
	        if(result.mrrg_yn == 'Y') {
				result.mrrg_yn = 'Y'; // 기혼('MEMBER_MARRIED_CD'의 기혼코드)	        	
	        } else if(result.mrrg_yn == 'N') {
	        	result.mrrg_yn = 'N'; // 미혼('MEMBER_MARRIED_CD'의 미혼코드)
	        }
	        $.fn.ifvmSetSelectOptionCommCode("mrrg_yn", "MEMBER_MARRIED_CD", result.mrrg_yn, null, false);
	        $.fn.ifvmSetSelectOptionCommCode("child_yn", "CHILD_YN", result.child_yn, null, false);

	        $.fn.ifvmSetSelectOptionCommCode("annvrsry_type_cd", "LOY_ANNVRSRY_TYPE_CD", result.annvrsry_type_cd, null, true);	// false
	        ...
	        $.fn.ifvmSetSelectOptionCommCode("snsTypeCd", "LOY_SNS_TYPE_CD", result.snsTypeCd, null, false);

			if($("#mbrDetailForm #certTypeCd").val() == "CI"){
				disArr = ['certTypeCd','certVal','getCert','cust_nm','gen_cd','email','emailBackSelet','hhp','hhpTop','hhpMid','hhpBot','birthdt','lunar_cd'];
				this.disabledId(disArr, true);
			}

			/* //구 회원일 경우 이름, 핸드폰 번호 disabled 해제
			if($("#mbrDetailForm #mbrTypeCd").val()=='M1'|| $("#mbrDetailForm #mbrTypeCd").val()=='M2'){
				disArr =['cust_nm','cust_nm_add','hhpTop','hhpMid','hhpBot']
				this.disabledId(disArr, false);
			} 
			disArr = ['certTypeCd','certVal','getCert','cust_nm','gen_cd','hhp','hhpTop','hhpMid','hhpBot','birthdt','lunar_cd','cust_nm_add','empNo','drmncy_chng_unit'];
            this.disabledId(disArr, true); */

            /*구 회원일 경우 이름, 핸드폰 번호 disabled 해제*/
			if( ($("#mbrDetailForm #mbrTypeCd").val()=='M1' || $("#mbrDetailForm #mbrTypeCd").val()=='M2') 
					&& $('#custTypeCd').val() == 'I' ) {
				disArr =['cust_nm','hhpTop','hhpMid','hhpBot']; //'cust_nm_add',
				this.disabledId(disArr, false);
				
				if( $("#mbrDetailForm #custstatcd").val()=='D' ){
					disArr = ['email','telTop','telMid','telBot','bunji','emailBackSelet','bunji_add','mrrg_yn','child_yn','annvrsryMm','annvrsryDd','snsTypeCd','snsAddr','C3','C6','C7','drmncy_chng_unit','certTypeCd'];
		            this.disabledId(disArr, true);
				}
			} else {
				if( $("#mbrDetailForm #custstatcd").val()=='D' ){
					$("#addCustBtn").attr('disabled',true);
				}
				disArr = ['certTypeCd','certVal','getCert','cust_nm','gen_cd','hhp','hhpTop','hhpMid','hhpBot','birthdt','lunar_cd','cust_nm_add','empNo','drmncy_chng_unit'];
	            this.disabledId(disArr, true);
			}
			...
			if(result.custStatCd=='D' && result.custtypecd=='I') {
				$("#mbrDetailForm #hhpTop").val('');
				$("#mbrDetailForm #hhpMid").val('');
				$("#mbrDetailForm #hhpBot").val('');
				$("#mbrDetailForm #hhp").val('');
				$("#mbrDetailForm #cust_nm").val('');
			}
		...
		if(custValidation.confirm() && mbrCValidation.confirm()){
			if(this.certCheck() == true) {
				...
				if(confirm('<spring:message code="수정하시겠습니까?"/>')){
					if( ($("#mbrDetailForm #mbrTypeCd").val()=='M1' || $("#mbrDetailForm #mbrTypeCd").val()=='M2') 
							&& $('#custTypeCd').val() == 'I' && $("#mbrDetailForm #custstatcd").val()=='D' ) {
						var url = '<ifvm:action name="saveMbrDorDetailPop"/>';
					}else {
					var url = '<ifvm:action name="saveMbrDetailPop"/>';
					}
	...
	this.setSelectField = function(values) { //select box 값 매핑
    	...
    	var custstatcd = ( values && values.custstatcd ) ? values.custstatcd : null;
        ...
        var child_yn = ( values && values.child_yn ) ? values.child_yn : null;
        $.fn.ifvmSetSelectOptionCommCode("custtypecd", "LOY_CUST_TYPE_CD", custtypecd, null, true);
        $.fn.ifvmSetSelectOptionCommCode("custstatcd", "LOY_CUST_STAT_CD", custstatcd, null, true);
        ...
        if(mrrgYn == 'Y') {
        	mrrgYn = 'Y'; // 기혼('MEMBER_MARRIED_CD'의 기혼코드)	        	
        } else if(mrrgYn == 'N') {
        	mrrgYn = 'N'; // 미혼('MEMBER_MARRIED_CD'의 미혼코드)
        }
        $.fn.ifvmSetSelectOptionCommCode("mrrg_yn", "MEMBER_MARRIED_CD", mrrgYn, null, false);
        $.fn.ifvmSetSelectOptionCommCode("child_yn", "CHILD_YN", child_yn, null, false);    
        
        $.fn.ifvmSetSelectOptionCommCode("annvrsry_type_cd", "LOY_ANNVRSRY_TYPE_CD", annvrsryTypeCd, null, true);//false	
...
// 			}
	...
	this.getCustItemValCk = function(){ // 대표 프로그램의 고객식별항목
		$.ifvSyncPostJSON('<ifvm:action name="custItemConfVal"/>',{pgmNo : pgmNo,custTypeCd : $('#custTypeCd').val() /* repYn : 'Y' */}, function(result){
			...
			for(var i=0;i <newList.length; i++) {
				...
				if( fieldId != "bunji" && fieldId != "bunji_add" && fieldId != "lunar_cd" && 
						fieldId != "zip" && fieldId != "zip_add" && fieldId != "user_id" && fieldId != "user_pwd"){ 
					if(newList[i].requiredYn =='Y'){
						if(newList[i].fieldNm!='GEN_CD') {
							if(newList[i].fieldNm=='HHP') {
								this.setRequired(["hhpTop"], true);
								this.setRequired(["hhpMid"], true);
								this.setRequired(["hhpBot"], true);
								fieldNm = '<span class="asterisk">*</span>'+"휴대전화";
							} else {
								requiredArr = [fieldId];
...
// 			else if(i == "zip") $("#mbrDetailForm #zipArea .input-group-addon").hide()
	...
	this.certChange = function(){
		...
		if(selectItem == 'NO_CERT'){
			...
			disArr = [ 'cust_nm', 'email', 'emailBackSelet', 'hhpTop', 'hhpMid', 'hhpBot', 'birthdt', 'lunar_cd','telTop','telMid','telBot'];
			this.disabledId(disArr, false);
...
$(document).ready(function(){
	...
	// '결혼여부','자녀유무' 필드에서, null인 경우의 option 텍스트를 '선택안함'으로 표기
	var childYnVarNull = document.querySelector('#child_ynArea #child_yn option[value=""]');
	childYnVarNull.innerText = '선택안함';
	var mrrgYnVarNull = document.querySelector('#mrrg_ynArea #mrrg_yn option[value=""]');
	mrrgYnVarNull.innerText = '선택안함';

	// 기념일 필드
	$('#annvrsry_type_cdArea').find('label').text('기념일유형'); // 기념일 필드 레이블 -> '기념일유형' 으로 수정
	$('#annvrsry_type_cd option[value="00"]').attr("selected", true); // 기념일은 무조건 '미지정' 상태
	
	// 음력/양력 select 필드
	$('#lunar_cd option[value="20"]').attr("selected", true); // 무조건 양력 선택(음력사용안함)
	$('#lunar_cd').attr("disabled", true);
	...
	if($("#mbrStatCd").val()!='활성' &&  $("#mbrStatCd").val()!='휴면') {
		$("#addCustBtn").remove();
	}
})
</script>