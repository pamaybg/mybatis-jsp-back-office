...
function transactionDetailDataLoad(testViewList_rid) {
 $.ifvSyncPostJSON('<ifvm:action name="getTransactionDetail"/>', {
...
function tabContent(){
	var testTab = $('#tabContent').DrawTab({
		item: [
			{label: '<spring:message code="L00598"/>', href: '<ifvm:url name="transAddInfo"/>'},
/* 			{label: '<spring:message code="L00599"/>', href: '<ifvm:url name="transPromoListNew"/>'}, */
			{label: '적립정책 항목', href: '<ifvm:url name="transPromoListNew"/>'},
			{label: '<spring:message code="Q02912"/>', href: '<ifvm:url name="transSearchTestInfoNew"/>'},
		]
	});
}
...
$(document).ready(function(){
	$.ifvmLnbSetting('transactionListNew');
	commCodeSetting();
	numberFormatter();
	transactionDetailDataLoad(testViewList_rid);


 $.ifvmLnbSetting('testViewListNew');
