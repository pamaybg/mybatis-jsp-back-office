...
function getAdrSearchResultGrid() {
	...
	colModel.push({name:'road_address.zone_no', 		index:'road_address.zone_no', 		searchable : false, resizable : false, align:'center', 	width:'50px'});
	colModel.push({name:'road_address.address_name', 	index:'road_address.address_name', 	searchable : false, resizable : false, align:'left', 	width:'120px'});
	colModel.push({name:'address.address_name', 		index:'address.address_name', 		searchable : false, resizable : false, align:'left', 	width:'120px'});
...
function adrSaveFn() {
    ...
    if (len > 0) {
		...
		//검색결과 세팅
		$('#zip, #custValidForm #zip').val(selectedRowData["road_address.zone_no"]); //주소 우편번호
    	$('#adres, #custValidForm #adres').val(selectedRowData["road_address.address_name"]); //주소
    	$('#roadNmAdrZipCd').val(selectedRowData["address.zip_code"]); //도로명주소 우편번호
    	$('#roadNmAdr').val(selectedRowData["address.address_name"]); //도로명주소
    	$('#bCode').val(selectedRowData["address.b_code"]); //법정코드
    	$('#bulidingCode').val(selectedRowData["road_address.main_building_no"]); //건물번호
	        ...
	        if ($.fn.ifvmIsNotEmpty(result)) {
	        	if(result.success) {
	        		...
	        		if(result.meta.total_count === 0) {
	        			...
	        			var adrSearchResultList = []; // 임시 그릇 배열
	        			
	        			$.each(result.documents, function(i, obj){
	        				// result.documents안의 각 객체에서, road_address 프로퍼티가 null이면(즉,도로명주소가없으면) 현재 폐기된 주소임
							if(obj.road_address != null) { // 현존하고 있는 주소만 고른다
								adrSearchResultList.push(obj);
							}
	        			});
	        			adrSearchResult = adrSearchResultList; // 전역에 있는 adrSearchResult로 완성된 배열을 투입
	        		}
	        	} else {
	        		if(result.message == 1 || result.message == 2 || result.message == 3) {
	        			$.ifvErrorPop(ifv.errorMessage.e_200400); //클라이언트 요청전문이 잘못되었습니다
