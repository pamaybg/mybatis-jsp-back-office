...
function loadOfferDtail(campaignOfferId) {
    //if (strType == 'LOY') {
	   	$.fn.ifvmLoadUiSetting("benefitsDetailArea", '<ifvm:url name="loyOfferDetailLoad"/>?id=' + campaignOfferId);
    //}
    //else {
   	    		...
   	    		if (detail == true) {
    		    	...
    		    	if ($.fn.ifvmIsNotEmpty(result)) {
    		    		...
    		    		if(checkSelected == true){
    		    			saveResult = result.success;
    		    			campaignOffer.offerList[0].offerId = selectMessage;
...
function benefitsOfferListPopSelect() {
	...
	if (len > 0) {
	    ...
	    $.ifvSyncPostJSON('<ifvm:action name="getLoyOfferDetail"/>', {
			...
			selectMessage= benefitsOfferListPopGrid.getCheckedGridData()[0].id;
	        setDetail(result);
