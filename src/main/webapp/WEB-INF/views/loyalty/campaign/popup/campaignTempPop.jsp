...
function campaignTemplateSelect() {
	...
	$.ifvSyncPostJSON('<ifvm:action name="getLoyTemp"/>',
			{camTypeCd : camTypeCd, camPrpsCd : camPrpsCd}, 	 
			function(result){
...
function getCampaignStatusList2(id) {
	...
	var firstLoad = true;	//제목,설명, 기본 셋팅은 한번만
 	$.ifvSyncPostJSON('<ifvm:action name="getLoyTempNodeList"/>',
			{tempMstId : id}, 	 
			function(result){
				$(result.rows).each(function(index, item){						
					if(firstLoad == true){
...
function templateSelect() {
	...
	if ($.fn.ifvmIsNotEmpty(copyCamId)) {
		var url = '<ifvm:url name="campaignWorkFlowLoy"/>?id=' + copyCamId + '&type=' + strType;
	    qtjs.href(url);
	} else {
...
<div class="campaign_temp_box" id="${'${'}tempMstId}" workflowid="${'${'}workflowId}" camid="${'${'}campaignId}" onclick="selectTemp(this)">
...
<div id="campaignTemplatePop">
	<div class="pop_inner_wrap">
		<div class="group_box_bg form-horizontal row">
