...
function contentsDesignPopInit() {
    ...
    //cDialog.model.width = "1300";
    cDialog.model.close = "closeContentsDesignPop";
	$.fn.ifvmSetSelectOptionCommCode("exeType", "PROM_EXE_TYPE", null, null, true);
    ...
    $("#chnlExeDtArea").show();
	contentsDesignPopCalendarSet();
	$('#chnlExeDtArea #exeTm').val('');
	$('#chnlExeDtArea #exeTm').attr('disabled',true);
}
...
function loadContentsDesign(){
    ...
    $.ifvSyncPostJSON('<ifvm:action name="getLoyaltyContentsChnnelRel"/>', {
        id: id
    },
    function(result) {
        if ($.fn.ifvmIsNotEmpty(result)) {
            ...
            //$("#exeDt").val(result.exeDt);
            $("#exeType").val(result.exeType);
            exeTypeChange();
            $("#exeTm").val(result.exeTm);
            // 실행일시 설정
    ...
    $.ifvSyncPostJSON('<ifvm:action name="getChannelByCodeName"/>',{
        ...
        if ($.fn.ifvmIsNotEmpty(result)) {
            ...
            if ($.fn.ifvmIsNotEmpty(url)) {
                ...
                var fileName = i[i.length-1];
                fileName = fileName.split('.')[0];
                url = '${pageContext.request.contextPath}/marketing/content/load/' + fileName + '?chnl=' + chnnelType +'&id='+contentsId + '&openType=WF'+'&type=prom';
            }
        }
    });
...
function contentsDesignSave(type) {
    ...
    if ($.fn.ifvmIsNotEmpty(contentsId) && contentsId != "null") {
        ...
        campaignContents.exeType = exeType;
        campaignContents.exeTm = exeTm;
        /* if (exeTm != null) {
    		campaignContents.exeTm = exeTm;
    	} */
        //call 캠페인 컨텐츠 채널 관계 저장
        $.ifvSyncPostJSON('<ifvm:action name="saveLoyaltyContentsChnnelRel"/>',
            campaignContents,
        function(result) {
            //set actual task ID
...
function exeTypeChange() {
	if($("#exeType").val()=='D') {
		$("#exeTm").val('');
		$("#exeTm").attr('disabled',true);
	} else {
		$("#exeTm").attr('disabled',false);
	}
}
...
<div id="dialogPopupWrap">
    ...
    <div class="pop_inner_wrap form-horizontal">
        <div class="page_btn_area" >
            ...
            <div class="col-xs-2">
                <button class="btn_Load" id="loadContentsBtn" objCode="loadContentsBtn_OBJ">
            ...
            <label class="col-xs-3 control-label"><span class="asterisk">*</span>배치 실행유형</label>
			<div class="col-xs-2">
				<ifvm:input type="select" names="exeType" required="true" id="exeType" />
			</div>
			<label class="col-xs-1 control-label"><span class="asterisk">*</span>실행일시</label>
