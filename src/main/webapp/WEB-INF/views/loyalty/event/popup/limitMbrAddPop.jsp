...
<div class="form-horizontal underline top_well" id="limitMbrAddForm">
	...
	<!-- 사유 -->
	<div class="row qt_border">
		<ifvm:inputNew type="textarea" id="limitDesc" label="L01633" labelClass="3" conClass="7" rows="5" required="true"/>	
	</div>
</div>
<div class="pop_btn_area">
	...
	<!-- 닫기 -->
	<button class="btn btn-sm btn_gray btn_lightGray2" id="limitUgcAddCancelBtn">
...
$(document).ready(function(){
	...
	selectUgcNm();//마스킹해제된 이름 가져오기
	
	setValidCalendar();//달력셋팅
	
	//닫기
	$("#limitUgcAddCancelBtn").on("click",function(){
		limitUgcAddPopupClose();
	});
...
//마스킹해제된 이름 가져오기
function selectUgcNm() {	
	if ($.fn.ifvmIsNotEmpty(ridMbr)) {
	    $.ifvSyncPostJSON('<ifvm:action name="ugcMbrClearMasking"/>', {
	    	ridMbr: ridMbr
	    },
	    function(result) {
	    	$('#custName').val(result.custNM);
	    });
	}
}
