...
function unRgstCardUploadDetailSetting(){
	if(testViewList_rid!=null){
		$.ifvSyncPostJSON('<ifvm:action name="getUnRgstCardUploadDetail"/>', {
			...
			//내가 요청한 건만 승인 요청 취소 가능
			 if(result.ridRqt != $.ifvmGetUserInfo().empId){
				 $("#cancelInsBtn").prop('disabled',true);
			 }
			//내가 요청한 건만 접수 취소, 승인요청 가능
			 if(result.createBy != $.ifvmGetUserInfo().id){
				 $("#cancelRequestBtn, #mainVanConfirmReqBtn").prop('disabled',true);
			 }
		});
	...
	$("#headForm button").prop('disabled',false);
	buttonValid();
}
...
function buttonValid(){
	...
	if($("#preSaveTrtStatus").val()!="50" && $("#preSaveTrtStatus").val()!="65"){
...
<div class="page-title">
	<h1>
	   <spring:message code="L02064"/>&gt;  <spring:message code="L00079"/>
	</h1>
</div>

<div class="page_btn_area" id="headForm">
	<div class="col-xs-4">
		<span> <spring:message code="L02065" /></span>
	</div>

	<div class="col-xs-8 searchbtn_r">
		<ifvm:inputNew type="button"  text="L02067"  id="cancelRequestBtn" btnFunc="fnCancelRequest" objCode="unRgstCardDetailcancelRequestBtn_OBJ"/>
		<ifvm:inputNew type="button"  text="M00962"  id="mainVanConfirmReqBtn" btnFunc="mainVanConfirmReq" objCode="unRgstCardDetailmainVanConfirmReqBtn_OBJ"/>
		<ifvm:inputNew type="button" text="L02059" id="cancelInsBtn" btnFunc="fnCancelIns" objCode="unRgstCardDetailcancelInsBtn_OBJ"/>
		<ifvm:inputNew type="button"  btnType="save"  text="L01695"  id="unRgstCardUploadSaveBtn" btnFunc="unRgstCardUploadSaveFn" objCode="unRgstDetailunRgstCardUploadSaveBtn_OBJ"/>
		<ifvm:inputNew type="button"  text="L01698"  id="litBtn" btnFunc="unRgstCardUploadListFn" objCode="unRgstDetaillitBtn_OBJ"/>
	</div>
	<script>
	...
	function fnCancelRequest(){
 		...
 		$.ifvSyncPostJSON('<ifvm:action name="cancelUnRgstUploadRequest"/>',
 			submitData, function(result) {
...
<div class="page_btn_area" id="headForm">
	<div class="col-xs-4">
	 ...
	 <div class="col-xs-8 searchbtn_r">
		...
		<ifvm:inputNew type="button"   btnType="search"  text="L01666"  id="allSearchBtn" btnFunc="allSearchFn" objCode="unRgstDetailallSearchBtn_OBJ"/>
		<ifvm:inputNew type="button"   btnType="search" text="L01667"  id="sucesSearchBtn" btnFunc="sucesSearchFn" objCode="unRgstDetailsucesSearchBtn_OBJ"/>
		<ifvm:inputNew type="button"   btnType="search" text="L01668"  id="failSearchBtn" btnFunc="failSearchFn" objCode="unRgstDetailfailSearchBtn_OBJ"/>
     </div>
     <script>
