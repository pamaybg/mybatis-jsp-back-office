...
/*
 * 1. 클래스명	: DatabaseDataAwsKmsEncryption
 * 2. 파일명	: DatabaseDataAwsKmsEncryption.java
 * 3. 패키지명	: com.icignal.security.dataencryption.impl
 * 4. 작성자명	: knlee
 * 5. 작성시간	: 2020. 1. 31. 오후 5:12:55
 */
 ...
 *	AWS KMS를 이용한 데이터베이스 데이터 키  구현 클래스	

 * </PRE>

 */ 
public class DatabaseDataAwsKmsEncryption implements DataEncryption {
	...
	public String getDataEncryptionKey(Environment env, StandardPBEStringEncryptor encryptor ) throws Exception {
        ...
        String fileName =    Optional.ofNullable(env.getProperty("icignal.security.data-encryption.database.key-file"))
        					.orElseThrow(() -> new NoSuchFieldError("application.properties 항목 [icignal.security.data-encryption.database.key-file]에 대한 value가 셋팅되어 있지 않습니다."));
        ...
        LogUtil.info("stringKey: " + stringKey);
    	} finally {
    		inputStream.close();
    	}
		return stringKey;
