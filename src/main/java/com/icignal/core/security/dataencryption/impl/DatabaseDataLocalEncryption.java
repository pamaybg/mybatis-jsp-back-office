...
public class DatabaseDataLocalEncryption implements DataEncryption {
	...
	public String getDataEncryptionKey(Environment env, StandardPBEStringEncryptor encryptor) throws Exception {
		...
		log.info("########## 데이터베이스 Data 암호화(Local) Key 생성  ##########");

		String fileName = Optional.ofNullable(env.getProperty("icignal.security.data-encryption.database.key-file"))
				.orElseThrow(() -> new NoSuchFieldError(
						"application.properties 항목 [icignal.security.data-encryption.database.key-file]에 대한 value가 셋팅되어 있지 않습니다."));
		...
		try { 
			...
			byte[] keyByte = DatabaseDataLocalEncryption.getBytes(inputStream);
	
			DatabaseDataLocalEncryption.keyObject = new SecretKeySpec(keyByte, "AES");
			if (DatabaseDataLocalEncryption.keyObject != null) {
				byte[] keyBate2 = DatabaseDataLocalEncryption.keyObject.getEncoded();
				...
				stringKey = Base64.encodeBase64String(DatabaseDataLocalEncryption.keyObject.getEncoded());
	...
	public static void main(String[] args) throws Exception {
		DatabaseDataLocalEncryption.makeKeyFile("db_key_dev.dat");
	}
	...
	private static void makeKeyFile(String fileName) throws Exception {
		...
		FileOutputStream fileOutputStream = new FileOutputStream("src/main/resources/security/encryption_key/" + fileName);
		try {
			// 키를 콘솔에서 입력받기
			...
			if (inputKey.getBytes().length != keyLength) {
				LogUtil.error("Key length must be " + keyLength + " bits.");
			}
	
			if (keyPassword.getBytes().length != keyLength) {
				LogUtil.error("Password length must be " + keyLength + " bits.");
			}
